...
**********************************************************************
*                                                                    *
* Vector quantization of atomic structures.                          *
*                                                                    *
**********************************************************************
...
static unsigned int NNMAX = 50;
static unsigned int MAXDT = 50 * 8;

static Rseq3 vglob;

/* Following will be Rseq3[NNMAX] */
static Rseq3 *wglob = 0;
static Rseq3 *average = 0;

/* Following will be Rseq3[MAXDT] */
static Rseq3 *vecdata = 0;

/* Following will be int[NNMAX] */
static int *tk = 0, *order = 0, *listcount = 0;

/* Following will be int[NNMAX][MAXDT] */
static int **list;

/* Following will be double[NNMAX] */
static double *effrad = 0;
static double *mcl = 0, *rmsd = 0;
static double *dist;
...

{
  ...
  if (argc != 3) {
    fprintf(stderr, "...qpdb> Usage: qpdb inputfile (PDB) outputfile (vectors)\n");
    exit(1);
  }
  ...
  printf("...qpdb> Found %d hydrogens, %d water atoms, %d codebook vectors, %d density atoms\n", num_hydrogen, num_water, num_codebook, num1 - num_nondens);
  ...
  if (num_hydrogen > 0) printf("...qpdb> Hydrogens will be ignored. \n");
  ...
  if (num_nondens < num1) { /* some density atoms found */
    ...
    printf("...qpdb> Vector Quantization of density atoms not possible, use qvol instead. Bye bye.\n");
    exit(1);
    ...
    nondens_mode = 1;
    if (num_water > 0) {
      printf("...qpdb> %d water atoms found in file %s \n", num_water, argv[1]);
      mass_mode = 1; /* if water found automated mass_weighting */
      printf("...qpdb> Mass-weighting on.\n");
      printf("...qpdb> Do you want to exclude the water atoms?\n");
      printf("...qpdb> \n");
      printf("...qpdb>      1: No\n");
      printf("...qpdb>      2: Yes\n");
      printf("...qpdb> ");
      water_mode = 2 - readln_int(); /* 0 = water ignored */
      if (!water_mode) printf("...qpdb> Water atoms will be ignored.\n");
    } else {
      water_mode = 0; /* water atoms ignored - redundant, no water found anyway */
      printf("...qpdb> Do you want to mass-weight the atoms ?\n");
      printf("...qpdb> \n");
      printf("...qpdb>      1: No\n");
      printf("...qpdb>      2: Yes\n");
      printf("...qpdb> ");
    ...
    printf("...qpdb> Do you want to select atoms based on a B-factor threshold?\n");
    printf("...qpdb> \n");
    printf("...qpdb>      1: No\n");
    printf("...qpdb>      2: Yes\n");
    printf("...qpdb> ");
    ...
    if (num_codebook > 0) {
      printf("...qpdb> %d codebook vectors found in file %s \n", num_codebook, argv[1]);
      printf("...qpdb> Do you want to exclude the codebook vectors?\n");
      printf("...qpdb> \n");
      printf("...qpdb>      1: No\n");
      printf("...qpdb>      2: Yes\n");
      printf("...qpdb> ");
  ...
  if (bfact_mode) {
    ...
    printf("...qpdb> Range of crystallographic B-factors: %5.2f - %5.2f.\n", minfact, maxfact);
    printf("...qpdb> Enter B-factor cutoff (only atoms below this value will be included): ");
    cutoff = readln_double();
    if (cutoff < minfact) {
      printf("...qpdb> No atoms selected. Try again. Bye bye.\n");
      exit(1);
    }
  }
  ...
  for (i = 0; i < num1; ++i) {
    if (num2 >= 1.3 * num1) {
      error_atom_count(60010, "qpdb", num2, num1);
    }
    if (!water_mode && check_water(pdb1, i)) continue;
    if (!hydrogen_mode && check_hydrogen(pdb1, i)) continue;
    if (!codebook_mode && check_codebook(pdb1, i)) continue;
  ...
  printf("...qpdb> %d equally weighted inputs out of originally %d atoms selected for conversion.\n", num2, num1);
  printf("...qpdb> \n");
  ...
  if (num2 < NNMIN) {
    fprintf(stderr, "...qpdb> Error: Internal number of atoms too small [e.c. 31040]\n");
    exit(31040);
  }

  /* from now on we work with pdb2, well, not really!) */
  ...
  cum /= (double)num2;
  cum = sqrt(cum);
  printf("...qpdb> Sphericity of the atomic structure:%5.2f \n", num2 / (0.60 * cum * cum * cum));


  /* select nn and exit loop if nn == 0 */
  printf("...qpdb> Enter desired number of codebook vectors for data quantization: (0 to exit): ");
  nn = readln_int();
  if (nn == 0) {
    printf("...qpdb> Bye bye.\n");
    exit(1);
  ...
  if (nn > NNMAX) {
    printf("...qpdb> Note: number of codebook vectors (%d) exceeds recommended maximum of %d!\n", nn, NNMAX);
  }
  if (nn < NNMIN) {
    nn = NNMIN;
    printf("...qpdb> Note: minimum number of codebook vectors (defined in situs.h): %d\n", nn);
  }
  ...
  for (currindex = 0; currindex < nn; ++currindex) {
    ...
    if (listcount[currindex] > 0) {
      ...
      rmsd[currindex] = -1;
      printf("...qpdb> Warning: Voronoi cell %d contains no data points.\n", currindex + 1);
      printf("...qpdb> Vector variability in output file will be set to 99.99.\n");
    }
  }
  ...
  fout = fopen(argv[2], "w");
  if (fout == NULL) {
    fprintf(stderr, "...qpdb> Error: Can't open file! %s  [e.c. 31100]\n", argv[2]);
    exit(31100);
  }

  /* compute equivalent spherical radius */

  /* compute closest vector for each atom */
  for (i = 0; i < num2; ++i) {
    k = 0;
    wdiff = 1e20;
    for (j = 0; j < nn; ++j) {
  ...
  for (i = 0; i < nn; ++i) {
    if (mcl[i] == 0) {
      fprintf(stderr, "...qpdb> Error: Voronoi cell %d contains no density [e.c. 31200]\n", i + 1);
      exit(31200);
  ...
  printf("...qpdb>\n");
  printf("...qpdb> Codebook vectors have been written to file %s\n", argv[2]);
  printf("...qpdb> The PDB B-factor field contains the equivalent spherical radii\n");
  printf("...qpdb> of the corresponding Voronoi cells (in Angstrom).\n");
  printf("...qpdb> Cluster analysis of the %d independent calculations:\n", MAXCYCLE);
  printf("...qpdb> The PDB occupancy field in %s contains the rms variabilities of the vectors.\n", argv[2]);

  if (skip == 0) {
    rmscum = 0;
    for (i = 0; i < nn; ++i) rmscum += rmsd[i];
    rmscum /= (double)nn;
    printf("...qpdb> Average rms fluctuation of the %d codebook vectors: %6.3f Angstrom\n", nn, rmscum);
  } else printf("...qpdb> Unable to compute average rms fluctuation, cluster analysis did not converge\n");

  /* write statistics of cluster size deviations*/
  k = 0;
  for (i = 0; i < nn; ++i) k += abs(listcount[i] - MAXCYCLE);
  if (k > 0) {
    printf("...qpdb> \n");
    printf("...qpdb> Warning: There are cluster size deviations:\n");
    printf("...qpdb> Expected cluster size: %d\n", MAXCYCLE);
    printf("...qpdb> Actual cluster sizes: ");
  ...
  rgyr /= (double)nn;
  rgyr = sqrt(rgyr);
  printf("...qpdb> Radius of gyration of the %d codebook vectors: %6.3f Angstrom\n", nn, rgyr);

  /* compute or write connectivities if desired */

  printf("...qpdb> \n");
  printf("...qpdb> Do you want to learn nearest-neighbor connectivities?\n");
  printf("...qpdb> Choose one of the following options -\n");
  printf("...qpdb>      1: No. \n");
  printf("...qpdb>      2: Learn and save to a PSF file\n");
  printf("...qpdb>      3: Learn and save to a constraints file\n");
  printf("...qpdb>      4: Learn and save to both PSF and constraints files\n");
  printf("...qpdb> ");
  psfmode = readln_int();
  ...
  switch (psfmode) {
    ...
    case 4:
      ...
      for (done = 0; done == 0;) {
        printf("...qpdb> Enter PSF filename: ");
        if (fgets(psf_file, FLENGTH, stdin) == NULL) {
          fprintf(stderr, "...qpdb> Error: Can't read filename [e.c. 31300]\n");
          exit(31300);
        }
        removespaces(psf_file, FLENGTH);
        fout = fopen(psf_file, "r");
        if (fout != NULL) {
          printf("...qpdb> Warning: File exists: %s \n", psf_file);
          printf("...qpdb> Do you want to overwrite? (yes/no) ");
          ch1 = getchar();
          for (;;)  {
            ch2 = getchar();
            if (ch2 == EOF) {
              fprintf(stderr, "...qpdb> Error: EOF while reading input [e.c. 31310]\n");
              exit(31310);
            }
            if (ch2 == '\n') break;
          }
      ...
      fprintf(fout, "\n");
      fclose(fout);
      printf("...qpdb> Connectivity data written to PSF file %s.\n", psf_file);
      if (psfmode == 2) break;
    ...
    case 3:
      ...
      /* read filename and open */
      for (done = 0; done == 0;) {
        printf("...qpdb> Enter connectivity filename: ");
        if (fgets(psf_file, FLENGTH, stdin) == NULL) {
          fprintf(stderr, "...qpdb> Error: Can't read filename [e.c. 31330]\n");
          exit(31330);
        }
        removespaces(psf_file, FLENGTH);
        fout = fopen(psf_file, "r");
        if (fout != NULL) {
          ...
          for (;;)  {
            ch2 = getchar();
            if (ch2 == EOF) {
              fprintf(stderr, "...qpdb> Error: EOF while reading input [e.c. 31340]\n");
              exit(31340);
            }
            if (ch2 == '\n') break;
          }
      ...
      fclose(fout);
      printf("...qpdb> Connectivity data / distances for all vectors written to file %s.\n", psf_file);
      break;
  }
  ...
  printf("...qpdb> \n");
  printf("...qpdb> Do you want to save the Voronoi cells?\n");
  printf("...qpdb> Choose one of the following options -\n");
  printf("...qpdb>      1: No. I'm done \n");
  printf("...qpdb>      2: Yes. Save cells to a PDB file\n");
  printf("...qpdb> ");
  vormode = readln_int();
  ...
  switch (vormode) {
    ...
    case 2:
      ...
      /* read filename and open */
      for (done = 0; done == 0;) {
        printf("...qpdb> Enter Voronoi cell PDB filename: ");
        if (fgets(pdb_file, FLENGTH, stdin) == NULL) {
          fprintf(stderr, "...qpdb> Error: Can't read filename [e.c. 31400]\n");
          exit(31400);
        }
        ...
        if (fout != NULL) {
          ...
          for (;;)  {
            ch2 = getchar();
            if (ch2 == EOF) {
              fprintf(stderr, "...qpdb> Error: EOF while reading input [e.c. 31410]\n");
              exit(31410);
            }
            if (ch2 == '\n') break;
          }
      ...
      printf("...qpdb> Voronoi cell numbers written to B-factor field in file %s.\n", pdb_file);
      break;
  }
...
/*====================================================================*/
void alloc_codebook_storage(unsigned int n)
{
  NNMAX = n;
