...

{
  ...
  if (argc == 4) {
    ...
    nn = readln_int();
    if (nn > NNMAX) {
      printf("...qvol> Note: number of codebook vectors (%d) exceeds recommend maximum of %d!\n", NNMAX, nn);
    }
    if (nn < NNMIN) {
      nn = NNMIN;
  ...
  switch (distmode) {
    ...
    case 4:
      ...
      rewind(fin);
      for (i = 0; i < numshake; i++) fscanf(fin, "%d %d %le", &j, &k, &dtemp);
  ...
  switch (psfmode) {
    ...
    case 5:
      ...
      if (numshake == 0) {
        printf("...qvol> No constraints loaded. Bye bye!\n");
        exit(1);
      }
      /* read filename and open */
      for (done = 0; done == 0;) {
