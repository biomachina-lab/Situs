...
int main(int argc, char **argv)
{
  ...
  freeVectAndZero(&phi_ga_save);
  freeVectAndZero(&phi_fx_save);
  ...
  freeVectAndZero(&g_phi_lo);

  /* set up FFTW grid sizes (deviate from our FFT grid for numerical reasons, see FFTW manual) */
  g_fftw_nvox_r2c = g_extz * g_exty * (2 * (g_extx / 2 + 1));
  g_fftw_nvox_c2r = g_extz * g_exty * (g_extx / 2 + 1);

  /* now re-allocate g_phi_lo on FFTW grid and copy content from padded map */
  ...
  /* free memory */
  freeVectAndZero(&g_fftw_grid_a);
  freeVectAndZero(&g_fftw_grid_b);
  ...
  freeVectAndZero(&g_phi_hi);
  freeVectAndZero(&g_phi_lo);
  freeVectAndZero(&g_phi_du);
  freeVectAndZero(&g_pdb_original);
  freeVectAndZero(&g_pdb_move);
  freeVectAndZero(&g_pdb_save);
  freeVectAndZero(&g_eulers);
  freeVectAndZero(&g_inside_list);
  freeVectAndZero(&g_inside_list_flipped);
...
static void *search6d_fft_par(void *thread_arg)
{
  ...
  freeVectAndZero(&p_phi_du);
  freeVectAndZero(&p_phi_hi);
  freeVectAndZero(&p_pdb_move);
  freeVectAndZero(&p_fftw_grid_b);
...
static void powell_optimization(void *args)
{
  ...
  freeVectAndZero(&pow_scratch->phi_hi);
  freeVectAndZero(&pow_scratch->phi_du);
  freeVectAndZero(&pow_scratch->pdb);
  freeVectAndZero(&pow_scratch);
  ...
  while (curr_iter != NULL) {
    last_iter = curr_iter;
    curr_iter = curr_iter->next;
    freeVectAndZero(&(last_iter->res));
    freeVectAndZero(&last_iter);
  }

  freeVectAndZero(&args);
...
static void create_inside_molecule_poslist_flipped(double *phi)
{
  ...
  freeVectAndZero(&mask_inside3); // THIS LINE HAS BEEN MODIFIED
}
