...
int main(int argc, char *argv[]) {
  ...
  for (i=0;i<num2;++i) {
    ...
    *(phi2+g2idz(z0,y0,x0)) += pdb2[i].weight * a * b * c; varp += pdb2[i].weight * a * b * c * ((1-a)*(1-a)+(1-b)*(1-b)+(1-c)*(1-c));
    *(phi2+g2idz(z1,y0,x0)) += pdb2[i].weight * a * b * (1-c); varp += pdb2[i].weight * a * b * (1-c) * ((1-a)*(1-a)+(1-b)*(1-b)+c*c);
    *(phi2+g2idz(z0,y1,x0)) += pdb2[i].weight * a * (1-b) * c; varp += pdb2[i].weight * a * (1-b) * c * ((1-a)*(1-a)+b*b+(1-c)*(1-c));
    *(phi2+g2idz(z0,y0,x1)) += pdb2[i].weight * (1-a) * b * c; varp += pdb2[i].weight * (1-a) * b * c * (a*a+(1-b)*(1-b)+(1-c)*(1-c));
    *(phi2+g2idz(z1,y1,x0)) += pdb2[i].weight * a * (1-b) * (1-c); varp += pdb2[i].weight * a * (1-b) * (1-c) * ((1-a)*(1-a)+b*b+c*c);
    *(phi2+g2idz(z0,y1,x1)) += pdb2[i].weight * (1-a) * (1-b) * c; varp += pdb2[i].weight * (1-a) * (1-b) * c * (a*a+b*b+(1-c)*(1-c));
    *(phi2+g2idz(z1,y0,x1)) += pdb2[i].weight * (1-a) * b * (1-c); varp += pdb2[i].weight * (1-a) * b * (1-c) * (a*a+(1-b)*(1-b)+c*c);
    *(phi2+g2idz(z1,y1,x1)) += pdb2[i].weight * (1-a) * (1-b) * (1-c); varp += pdb2[i].weight * (1-a) * (1-b) * (1-c) * (a*a+b*b+c*c);
  ...
  case 1:
    ...
    reso = 2.0*sig1; if (corr_mode == 2) reso = 2.0*sqrt(sig1*sig1+varp*width*width); /* variances are additive for uncorrelated samples */
    varmap = kmsd; if (corr_mode == 1) varmap -= varp; /* variances are additive for uncorrelated samples */
  ...
  case 2:
    ...
    reso = 2.0*sig2; if (corr_mode == 2) reso = 2.0*sqrt(sig2*sig2+varp*width*width); /* variances are additive for uncorrelated samples */
    varmap = kmsd; if (corr_mode == 1) varmap -= varp; /* variances are additive for uncorrelated samples */
	...
	for (indx=0;indx<g_extx;indx++) {
	  ...
	  dval = kampl * (1.0 - dsqu * bvalue); if (dval<0.0) dval = 0.0;
  ...
  case 3:
    ...
    reso = 2.0*sig3; if (corr_mode == 2) reso = 2.0*sqrt(sig3*sig3+varp*width*width); /* variances are additive for uncorrelated samples */
    varmap = kmsd; if (corr_mode == 1) varmap -= varp; /* variances are additive for uncorrelated samples */
  ...
  case 4:
    ...
    reso = 2.0*sig4; if (corr_mode == 2) reso = 2.0*sqrt(sig4*sig4+varp*width*width); /* variances are additive for uncorrelated samples */
    varmap = kmsd; if (corr_mode == 1) varmap -= varp; /* variances are additive for uncorrelated samples */
  ...
  case 5:
    ...
    reso = 2.0*sig5; if (corr_mode == 2) reso = 2.0*sqrt(sig5*sig5+varp*width*width); /* variances are additive for uncorrelated samples */
    varmap = kmsd; if (corr_mode == 1) varmap -= varp; /* variances are additive for uncorrelated samples */
