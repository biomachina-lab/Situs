...
static void zero_pixel_in_all_slices (int, int, int, int, int, int, int, int, double *pphi);
static void get_crop_parameters(int *, int *, int *, int *, int *, int *, int, int, int);
static void get_pad_parameters(int *, int *, int *, int *, int *, int *, int, int, int);
static void update_parameters(int , double **, int *, int *, int *, unsigned *, unsigned *, unsigned *, 
			      double *, double *, double *, int , int , int , 
			      int , int , int, double, int *, double *, int *, double *, unsigned long *);
static FILE* get_save_file_info(int, char *, char *);
static int neighbor (char *, int, int, int, int, int, int);
static void flood_fill_nonrecursive (double *, char *, double, int, int, int, int, int, int);
...
int main (int argc, char *argv[]) { 
  ...
  switch (slicemode) {
  case 1:
    rangea = extx; rangeb = exty; rangec = extz; cha = 'x'; chb = 'y'; chc = 'z';
    break;
  case 2:
    rangea = extz; rangeb = extx; rangec = exty; cha = 'z'; chb = 'x'; chc = 'y';
    break;
  case 3:
    rangea = exty; rangeb = extz; rangec = extx; cha = 'y'; chb = 'z'; chc = 'x';
    break;
  default:
  ...
  for (done=0;done==0;) {
    ...
    if (k>=0) { /* cross section */
      ...
      for(i=0;i<rangea;i+=stride) printf(" "); printf ("  %c\n",chb);
      for(i=0;i<rangea;i+=stride) printf(" "); printf ("  ^\n");
	...
	get_crop_parameters(&minx, &miny, &minz, &maxx, &maxy, &maxz, extx, exty, extz);
	...
	get_pad_parameters(&minx, &miny, &minz, &maxx, &maxy, &maxz, extx, exty, extz);
	project_map_lattice(&pphi2, maxx-minx+1, maxy-miny+1, maxz-minz+1, 
			    origx+minx*width, origy+miny*width, origz+minz*width, width, width, width,
			    pphi, extx, exty, extz, origx, origy, origz, width, width, width);
	free_vect_and_zero_ptr(&pphi);
	...
	for(j=0;j<rangeb;++j)
	  for(i=0;i<rangea;++i) {
	    ...
	    case 1: /* clip inside, including edges */
	      if (check_polygon!='o')
		zero_pixel_in_all_slices (i, j, indz, slicemode, extx, exty, extz, rangec, pphi);
	      break;
	...
	update_parameters(slicemode, &ppro, &rangea, &rangeb, &rangec, &extx, &exty, &extz, &origx, &origy, &origz, 
			  extx-1, 0, exty-1, 0, extz-1, 0, width, &k, &cutoff, &stride, pphi, &nvox);
	break;
	...
	xstart = readln_int()-1; if (xstart<0) xstart = 0; if (xstart>=extx) xstart = extx-1;
	printf ("voledit> Enter floodfill start index y (1-%d): ", exty);
	ystart = readln_int()-1; if (ystart<0) ystart = 0; if (ystart>=exty) ystart = exty-1;
	printf ("voledit> Enter floodfill start index z (1-%d): ", extz);
	zstart = readln_int()-1; if (zstart<0) zstart = 0; if (zstart>=extz) zstart = extz-1;
	...
	flood_fill_nonrecursive(pphi,iphi,cutoff,xstart,ystart,zstart,extx,exty,extz);

	/* find neighbor elements of filled volume (these have values below cutoff) */
	for (indz=0;indz<extz;indz++)
	  for (indy=0;indy<exty;indy++)
	    for (indx=0;indx<extx;indx++)
	      if (neighbor(iphi,indx,indy,indz,extx,exty,extz)) *(iphi+gidz_general(indz,indy,indx,exty,extx)) = 2;
	...
	for (count=0;count<nvox;count++) {
	  ...
	  else if (*(iphi+count)==2 && *(pphi+count)<DEFAULT_ZERO) *(pphi+count)=DEFAULT_ZERO;
	}
...
static void zero_pixel_in_all_slices (int i, int j, int indz, int slicemode, int extx, int exty,
				      int extz, int rangec, double *pphi) {
...
static void update_parameters(int slicemode, double **ppro, int *rangea, int *rangeb, int *rangec, unsigned *extx, 
			      unsigned *exty, unsigned *extz, double *origx, double *origy, double *origz, 
			      int maxx, int minx, int maxy, int miny, int maxz, int minz, double width,
			      int *k, double *cutoff, int *stride, double *pphi, unsigned long *nvox) {
  ...
  *origz+= minz*width;
  ...
  switch (slicemode) {
  case 1:
    *rangea = *extx; *rangeb = *exty; *rangec = *extz;
    break;
  case 2:
    *rangea = *extz; *rangeb = *extx; *rangec = *exty;
    break;
  case 3:
    *rangea = *exty; *rangeb = *extz; *rangec = *extx;
    break;
  }
  *k = (*rangec)/2;
  *nvox = (*extx)*(*exty)*(*extz);
...
static void get_pad_parameters(int *minx, int *miny, int *minz, int *maxx, int *maxy, 
			       int *maxz, int extx, int exty, int extz) {
  ...
  *minx = 0-readln_int(); if (*minx>0) *minx = 0;
  printf ("voledit> Upper x margin (>= 0): ");
  *maxx = extx-1+readln_int(); if (*maxx<extx) *maxx = extx-1;
  printf ("voledit> Lower y margin (>= 0): ");
  *miny = 0-readln_int(); if (*miny>0) *miny = 0;
  printf ("voledit> Upper y margin (>= 0): ");
  *maxy = exty-1+readln_int(); if (*maxy<exty) *maxy = exty-1;
  printf ("voledit> Lower z margin (>= 0): ");
  *minz = 0-readln_int(); if (*minz>0) *minz = 0;
  printf ("voledit> Upper z margin (>= 0): ");
  *maxz = extz-1+readln_int(); if (*maxz<extz) *maxz = extz-1;
}

static void get_crop_parameters(int *minx, int *miny, int *minz, int *maxx, int *maxy, 
				int *maxz, int extx, int exty, int extz) {
  ...
  *minx = readln_int()-1; if (*minx<0) *minx = 0; if (*minx>=extx) *minx = extx-1;
  printf ("voledit> Maximum x value (%d-%d): ",*minx+1,extx);
  *maxx = readln_int()-1; if (*maxx<*minx) *maxx = *minx; if (*maxx>=extx) *maxx = extx-1;
  printf ("voledit> Minimum y value (1-%d): ",exty);
  *miny = readln_int()-1; if (*miny<0) *miny = 0; if (*miny>=exty) *miny = exty-1;
  printf ("voledit> Maximum y value (%d-%d): ",*miny+1,exty);
  *maxy = readln_int()-1; if (*maxy<*miny) *maxy = *miny; if (*maxy>=exty) *maxy = exty-1;
  printf ("voledit> Minimum z value (1-%d): ",extz);
  *minz = readln_int()-1; if (*minz<0) *minz = 0; if (*minz>=extz) *minz = extz-1;
  printf ("voledit> Maximum z value (%d-%d): ",*minz+1,extz);
  *maxz = readln_int()-1; if (*maxz<*minz) *maxz = *minz; if (*maxz>=extz) *maxz = extz-1;
}


static void flood_fill_nonrecursive (double *pphi, char *iphi, double cutoff,  int l, int m, int n, int extx, int exty, int extz) {
  ...
  while (list_head != NULL) {
	...
	if (y > 0) {
	  ...
	  if (z > 0) flood_fill_nonrecursive_push(&list_tail,x-1,y-1,z-1);
	  if ((z+1) < extz) flood_fill_nonrecursive_push(&list_tail,x-1,y-1,z+1);
	}
	if ((y+1) < exty) {
	  flood_fill_nonrecursive_push(&list_tail,x-1,y+1,z);
	  if (z > 0) flood_fill_nonrecursive_push(&list_tail,x-1,y+1,z-1);
	...
	if (z > 0) flood_fill_nonrecursive_push(&list_tail,x-1,y,z-1);    
	if ((z+1) < extz) flood_fill_nonrecursive_push(&list_tail,x-1,y,z+1);
      }
      if ((x+1) < extx) {
	flood_fill_nonrecursive_push(&list_tail,x+1,y,z);
	if (y > 0) {
	...
	if ((z+1) < extz) flood_fill_nonrecursive_push(&list_tail,x,y-1,z+1);
      }
      if ((y+1) < exty) {
	flood_fill_nonrecursive_push(&list_tail,x,y+1,z);
	if (z > 0) flood_fill_nonrecursive_push(&list_tail,x,y+1,z-1);
	if ((z+1) < extz) flood_fill_nonrecursive_push(&list_tail,x,y+1,z+1);
      ...
      if (z > 0) flood_fill_nonrecursive_push(&list_tail,x,y,z-1);    
      if ((z+1) < extz) flood_fill_nonrecursive_push(&list_tail,x,y,z+1);
...
static int neighbor(char *iphi, int l, int m, int n, int extx, int exty, int extz) {
  ...
  if (*(iphi+gidz_general(n,m,l,exty,extx)) == 1) return 0;

  if (l > 0) {
    if (*(iphi+gidz_general(n,m,l-1,exty,extx)) == 1) return 1;
    if (m > 0) {
      if (*(iphi+gidz_general(n,m-1,l-1,exty,extx)) == 1) return 1;
      if (n > 0) if (*(iphi+gidz_general(n-1,m-1,l-1,exty,extx)) == 1) return 1;
      if ((n+1) < extz) if (*(iphi+gidz_general(n+1,m-1,l-1,exty,extx)) == 1) return 1;
    }
    if ((m+1) < exty) {
      if (*(iphi+gidz_general(n,m+1,l-1,exty,extx)) == 1) return 1;
      if (n > 0) if (*(iphi+gidz_general(n-1,m+1,l-1,exty,extx)) == 1) return 1;
    ...
    if (n > 0) if (*(iphi+gidz_general(n-1,m,l-1,exty,extx)) == 1) return 1;
    if ((n+1) < extz) if (*(iphi+gidz_general(n+1,m,l-1,exty,extx)) == 1) return 1;
  }
  if ((l+1) < extx) {
    if (*(iphi+gidz_general(n,m,l+1,exty,extx)) == 1) return 1;
    if (m > 0) {
  ...
  if (m > 0) {
    if (*(iphi+gidz_general(n,m-1,l,exty,extx)) == 1) return 1;
    if (n > 0) if (*(iphi+gidz_general(n-1,m-1,l,exty,extx)) == 1) return 1;
    if ((n+1) < extz) if (*(iphi+gidz_general(n+1,m-1,l,exty,extx)) == 1) return 1;
  }
  if ((m+1) < exty) {
    if (*(iphi+gidz_general(n,m+1,l,exty,extx)) == 1) return 1;
    if (n > 0) if (*(iphi+gidz_general(n-1,m+1,l,exty,extx)) == 1) return 1;
  ...
  if (n > 0) if (*(iphi+gidz_general(n-1,m,l,exty,extx)) == 1) return 1;
  if ((n+1) < extz) if (*(iphi+gidz_general(n+1,m,l,exty,extx)) == 1) return 1;
  return 0;
}
